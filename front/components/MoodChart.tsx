...
const MoodChart = ({ userId }: MoodChartProps) => {
  ...
  // analytics에서 추세 가져오기
  const trendingUp = analytics?.trend === "improving" || 
  ...
  const averageScore = analytics?.average_score || 
    Math.round(
      (displayData.reduce((sum, d) => sum + d.score, 0) / displayData.length) * 10
    ) / 10;

  // 최근 개선도
  const recentTrend = analytics?.trend_percentage ||
    ((lastSevenDays.at(-1)?.score ?? 0) - (lastSevenDays.at(0)?.score ?? 0)) /
    (lastSevenDays.at(0)?.score ?? 1) * 100;
          ...
          <CardContent>
            <div
              className={`text-3xl font-bold ${
                trendingUp ? "text-green-600" : "text-orange-600"
              }`}
            >
              {isLoading ? "..." : `${trendingUp ? "↑" : "↓"} ${Math.abs(recentTrend).toFixed(1)}%`}
            </div>
            <p className="text-xs text-green-700 mt-1">
              {analytics?.trend === "improving" ? "개선 중" : analytics?.trend === "declining" ? "주의 필요" : "안정적"}
            </p>
          </CardContent>
        </Card>
        ...
        >
          ...
          >
            <CardHeader>
              ...
              <CardDescription>
                대시보드의 각 통계 지표가 어떻게 계산되는지 설명합니다
              </CardDescription>
            </CardHeader>
